...
public class ScrollContent : MonoBehaviour
{
    ...
    /// </summary>
    [SerializeField] private RectTransform[] rtChildren;
    ...
    private void Start()
    {
        ...
        rtChildren = new RectTransform[_stageList._stageList.Count];

        for (int i = 0; i < _stageList._stageList.Count; i++)
        {
            GameObject newStagePanel = Instantiate(_stagePanel, this.transform);
            newStagePanel.GetComponent<StageInfo>().Init(_stageList._stageList[i]);
            rtChildren[i] = newStagePanel.GetComponent<RectTransform>();
        }
